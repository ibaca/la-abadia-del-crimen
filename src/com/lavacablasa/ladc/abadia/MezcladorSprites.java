...
class MezcladorSprites {
    ...
    void mezclaSprites(Sprite[] sprites, int num) {
        ...
        for (i = 0; i < numSprites; i++) {
            ...
            for (int j = 0; j < numSprites; j++) {
                ...
                if (!recortaSprite(spr.posXTile, spr.anchoFinal, sprAMezclar.posXPant, sprAMezclar.ancho,
                        lgtudClipDist1Dist2X)) {
                    continue;
                }

                // comprueba si el sprite a mezclar puede verse en la zona del sprite que se está dibujando
                if (!recortaSprite(spr.posYTile, spr.altoFinal, sprAMezclar.posYPant, sprAMezclar.alto,
                        lgtudClipDist1Dist2Y)) {
                    continue;
                }
                ...
                dibujaTilesEntreProfundidades(spr, profMinX, profMinY, sprAMezclar.posXLocal + 1,
    ...
    // vuelca al buffer de sprites los tiles no dibujados que están detrás del sprite actual
    void dibujaTilesEntreProfundidades(Sprite spr, int profMinX, int profMinY, int profMaxX, int profMaxY, int desp,
            boolean ultimaPasada) {
        ...
        for (int j = 0; j < numTilesY; j++) {
            ...
            for (int i = 0; i < numTilesX; i++) {
                ...
                if (estaEnBufferTiles(bufTilesPosX + i, bufTilesPosY + j)) {
                    ...
                    GeneradorPantallas.TileInfo ti = juego.motor.genPant.bufferTiles[bufTilesPosY + j][bufTilesPosX
                            + i];
            ...
            for (int j = 0; j < 8; j++) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        bufferMezclas[dest] = (byte) CPC6128.unpackPixelMode1(data, k);
                        dest++;
                    }
            ...
            for (int j = 0; j < 8; j++) {
                ...
                for (int i = 0; i < 4; i++) {
                    ...
                    for (int k = 0; k < 4; k++) {
                        ...
                        bufferMezclas[dest] = (byte) ((oldColor & juego.motor.genPant.mascaras[numTabla + 1][color])
                                | juego.motor.genPant.mascaras[numTabla][color]);
                        dest++;
                    }
