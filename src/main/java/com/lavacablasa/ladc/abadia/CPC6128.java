...
public class CPC6128 {
    ...
    static {
        ...
        for (int i=0; i<256; i++) {
            MODE_0_DECODE[i] = (byte) (
                    (unpackPixelMode0(i, 0) << 0) |
                    (unpackPixelMode0(i, 1) << 4)
            );

            MODE_1_DECODE[i] = (byte) (
                    (unpackPixelMode1(i, 0) << 0) |
                    (unpackPixelMode1(i, 1) << 2) |
                    (unpackPixelMode1(i, 2) << 4) |
                    (unpackPixelMode1(i, 3) << 6)
            );
        }
    }
    ...
    public void showMode0Screen(byte[] data)
    {
        for (int y=0; y<200; y++) {
            ...
            for (int x=0; x<80; x++) {
                screenBuffer[targetLinePos + x] = data[sourceLinePos + x];
            }
        }
    }
    ...
    private static int[] MODE_1_BYTE_COLORS = { 0b00000000, 0b11110000, 0b00001111, 0b11111111 };
