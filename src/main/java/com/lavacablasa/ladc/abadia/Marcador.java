    ...
    void dibujaMarcador()
    {
        ...
        for (int j = 0; j < 32; j++){
            for (int i = 0; i < 256/4; i++){
                int value = juego.gameData(data);
                for (int k = 0; k < 4; k++){
                    juego.cpc6128.setMode1Pixel(32 + 4*i + k, 160 + j, juego.cpc6128.unpackPixelMode1(value, k));
                }
                data++;
            }
        }
                ...
                if ((objetos & (1 << (Juego.numObjetos - 1))) != 0){
                    ...
                    for (int j = 0; j < spr.alto; j++){
                        for (int i = 0; i < spr.ancho; i++){
                            int value = juego.gameData(data);
                            for (int k = 0; k < 4; k++){
                                juego.cpc6128.setMode1Pixel(posX + 4*i + k, posY + j, juego.cpc6128.unpackPixelMode1(value, k));
                            }
                            data++;
                        }
                    }
