...
public class TimingHandler {
    ...
    public TimingHandler(Timer timer, int intsPerSecond, int intsPerVideoUpdate, int intsPerLogicUpdate) {
        ...
        lastFrameBase =
                lastFpsTime - (int) (INTERRUPTS_PER_BASE_TIME_UPDATE * numInterruptsPerVideoUpdate * timePerInterrupt);
    }
    ...
    public void waitThisInterrupt() {
        ...
        if (throttle) {
            numIntsModeBaseTimeUpdateInts =
                    interruptNum % (INTERRUPTS_PER_BASE_TIME_UPDATE * numInterruptsPerVideoUpdate);
            speedThrottle();
        }
    ...
    private void speedThrottle() {
        ...
        if (numIntsModeBaseTimeUpdateInts == 0) {
            thisFrameBase = lastFrameBase + (int) (INTERRUPTS_PER_BASE_TIME_UPDATE * numInterruptsPerVideoUpdate
                    * timePerInterrupt);
        }
        ...
        // recalculate base values each INTERRUPTS_PER_BASE_TIME_UPDATE*numInterruptsPerVideoUpdate frames
        if (numIntsModeBaseTimeUpdateInts == 0) {
            lastFrameBase = currentTime - targetTime > ticksPerSleepMiliSec ? currentTime : targetTime;
        }
    }
}
